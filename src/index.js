...
const BackgroundSlider = ({
  ...
  const imgs = nodes.map((data, index) => {
    const findObject = (haystack, needle) => {
      const keys = Object.keys(haystack);
      const match = keys.find((key) => key === needle);
      if (keys && haystack[keys] && !match) {
        return findObject(haystack[keys], needle);
      } else if (match) {
